...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            <div class="bg-white rounded-[2rem] shadow-xl p-10 md:p-14 border border-slate-100 relative overflow-hidden">
                <div class="absolute top-0 right-0 w-48 h-48 bg-navy/[0.02] rounded-bl-[10rem] pointer-events-none"></div>
                
                <h2 class="text-3xl font-serif font-bold text-navy mb-12 relative z-10">Guest Details</h2>
                ...
                <form action="{{ route('bookings.store') }}" method="POST" class="space-y-10 relative z-10" id="bookingForm">
                    ...
                    <div class="space-y-10">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div>
                                <label for="guest_name" class="block text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-3 ml-1">Full Name</label>
                                <input type="text" name="guest_name" id="guest_name" 
                                       value="{{ old('guest_name', auth()->user()?->name ?? '') }}"
                                       class="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-xl focus:border-gold focus:outline-none transition-all text-navy text-sm placeholder-slate-400" placeholder="e.g. Alexander Hamilton" required>
                            </div>
                            <div>
                                <label for="guest_email" class="block text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-3 ml-1">Email Address</label>
                                <input type="email" name="guest_email" id="guest_email" 
                                       value="{{ old('guest_email', auth()->user()?->email ?? '') }}"
                                       class="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-xl focus:border-gold focus:outline-none transition-all text-navy text-sm placeholder-slate-400" placeholder="alex@salpat.com" required>
                            </div>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div>
                                <label for="guest_passport_id" class="block text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-3 ml-1">Passport / ID Number</label>
                                <input type="text" name="guest_passport_id" id="guest_passport_id" 
                                       value="{{ old('guest_passport_id') }}"
                                       class="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-xl focus:border-gold focus:outline-none transition-all text-navy text-sm placeholder-slate-400" placeholder="Identification Number" required>
                            </div>
                            <div>
                                <label for="guest_phone" class="block text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-3 ml-1">Phone Number</label>
                                <input type="tel" name="guest_phone" id="guest_phone" 
                                       value="{{ old('guest_phone', auth()->user()?->phone ?? '') }}"
                                       class="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-xl focus:border-gold focus:outline-none transition-all text-navy text-sm placeholder-slate-400" placeholder="+255 --- --- ---">
                            </div>
                        </div>

                        <div>
                            <label for="guest_address" class="block text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-3 ml-1">Sanctuary Delivery Address</label>
                            <textarea name="guest_address" id="guest_address" rows="3"
                                   class="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-xl focus:border-gold focus:outline-none transition-all text-navy text-sm placeholder-slate-400 resize-none" placeholder="Your residential or billing address" required>{{ old('guest_address', auth()->user()?->profile?->address ?? '') }}</textarea>
                        </div>
                    </div>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-[2rem] shadow-xl overflow-hidden border border-slate-100 sticky top-10">
                <div class="h-64 relative group overflow-hidden">
                    <img src="{{ asset('storage/' . $room->image) }}" class="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-700">
                    <div class="absolute inset-0 bg-navy/20"></div>
                    <div class="absolute bottom-6 left-6 right-6">
                        <h4 class="text-2xl font-serif font-bold text-white mb-1">{{ $room->name }}</h4>
