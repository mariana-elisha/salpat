...
<div class="space-y-8" x-data="{ showConsultModal: false, showIssueModal: false, selectedRoom: null, selectedRoomName: '', showAddModal: false, showUseModal: false, selectedItem: null, selectedItemName: '', selectedItemMax: 1 }">
    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($occupiedRooms as $room)
                <div class="group bg-slate-900/50 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-800 overflow-hidden hover:border-rose-500/50 transition-all duration-500 relative">
                    <div class="absolute top-0 left-0 w-1.5 h-full bg-rose-500"></div>
                    <div class="p-6 relative z-10">
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="bg-rose-500/10 text-rose-400 px-2 py-0.5 rounded-lg text-xs font-mono font-bold border border-rose-500/20">#{{ $room->room_number ?? 'N/A' }}</span>
                                    <span class="px-2 py-0.5 text-[10px] font-black rounded-full bg-rose-600 text-white uppercase tracking-wider">OCCUPIED</span>
                                </div>
                                <h3 class="text-lg font-bold text-white group-hover:text-rose-400 transition-colors">{{ $room->name }}</h3>
                                <p class="text-xs text-slate-400 capitalize">{{ $room->type }}</p>
                            </div>
                        </div>
                        
                        <div class="bg-slate-800/50 rounded-2xl p-4 border border-slate-700 mb-5 flex items-center justify-between">
                            <div>
                                <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-0.5">Time to Checkout</p>
                                <p class="text-xl font-black text-white">{{ $room->time_remaining ?? 'In-Stay' }}</p>
                            </div>
                            <div class="w-10 h-10 bg-slate-800 rounded-xl shadow-sm flex items-center justify-center text-rose-400 border border-slate-700">
                        ...
                        <div class="space-y-3">
                            <form action="{{ route('housekeeping.rooms.update', $room) }}" method="POST">
                                ...
                                <div class="flex gap-2">
                                    <select name="status" class="flex-1 rounded-xl border-slate-700 text-xs font-bold focus:border-rose-500 focus:ring-rose-500 py-2.5 px-3 bg-slate-800 text-white transition-all">
                            ...
                            <button @click="selectedRoom = {{ $room->id }}; selectedRoomName = '{{ $room->room_number ?? $room->name }}'; showConsultModal = true"
                                class="w-full bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white font-bold py-2.5 rounded-xl text-[10px] uppercase tracking-[0.2em] border border-slate-700 transition-all flex items-center justify-center gap-2">
    ...
    <div class="space-y-4">
        <div class="px-6 py-5 border-b border-slate-800 flex justify-between items-center bg-slate-800/30 rounded-t-2xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($theRest as $room)
                ...
                @endphp
                <div class="group bg-slate-900/50 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-800 overflow-hidden hover:border-{{ $accentColor }}-500/50 transition-all duration-500 relative">
                    <div class="absolute top-0 left-0 w-1.5 h-full bg-{{ $accentColor }}-500"></div>
                    <div class="p-6 relative z-10">
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="bg-{{ $accentColor }}-500/10 text-{{ $accentColor }}-400 px-2 py-0.5 rounded-lg text-xs font-mono font-bold border border-{{ $accentColor }}-500/20">#{{ $room->room_number ?? 'N/A' }}</span>
                                </div>
                                <h3 class="text-lg font-bold text-white group-hover:text-{{ $accentColor }}-400 transition-colors">{{ $room->name }}</h3>
                                <p class="text-xs text-slate-400 capitalize">{{ $room->type }}</p>
                            </div>
                            <span class="px-2.5 py-1 text-[10px] font-black rounded-full uppercase tracking-wider bg-{{ $accentColor }}-500/10 text-{{ $accentColor }}-400 border border-{{ $accentColor }}-500/20">{{ $label }}</span>
                        </div>
                        <div class="space-y-3">
                            <form action="{{ route('housekeeping.rooms.update', $room) }}" method="POST">
                                @csrf @method('PATCH')
                                <div class="flex gap-2">
                                    <select name="status" class="flex-1 rounded-xl border-slate-700 text-xs font-bold focus:border-{{ $accentColor }}-500 focus:ring-{{ $accentColor }}-500 py-2.5 px-3 bg-slate-800 text-white transition-all">
                                        <option value="clean" {{ $room->housekeeping_status == 'clean' ? 'selected' : '' }}>Clean / Ready</option>
                                        <option value="dirty" {{ $room->housekeeping_status == 'dirty' ? 'selected' : '' }}>Needs Cleaning</option>
                                        <option value="cleaning_in_progress" {{ $room->housekeeping_status == 'cleaning_in_progress' ? 'selected' : '' }}>In Progress</option>
                                    </select>
                            ...
                            <div class="flex gap-2">
                                <button @click="selectedRoom = {{ $room->id }}; selectedRoomName = '{{ $room->room_number ?? $room->name }}'; showConsultModal = true"
                                    class="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white font-bold py-2.5 rounded-xl text-[10px] uppercase tracking-[0.2em] border border-slate-700 transition-all flex items-center justify-center gap-2">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg>
                                    Consult
                                </button>
                                <button @click="selectedRoom = {{ $room->id }}; selectedRoomName = '{{ $room->room_number ?? $room->name }}'; showIssueModal = true"
                                    class="bg-rose-500/10 hover:bg-rose-500 text-rose-400 hover:text-white rounded-xl px-3.5 py-2.5 transition-all flex items-center justify-center border border-rose-500/20 shadow-sm" title="Report Issue">
                ...
                @empty
                    <div class="col-span-full bg-slate-900/30 rounded-2xl border-2 border-dashed border-slate-800 p-14 text-center text-slate-500 text-sm italic">No rooms found.</div>
                @endforelse
        </div>
    </div>
    ...
    <template x-if="showConsultModal">
        <div class="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <div class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm" @click="showConsultModal = false"></div>
            <div class="bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl w-full max-w-md relative z-10 overflow-hidden">
                <div class="bg-slate-800 p-6 text-white flex justify-between items-center border-b border-slate-700">
                    ...
                    <button @click="showConsultModal = false" class="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white transition-colors">
                ...
                <form :action="'{{ url('housekeeping/rooms') }}/' + selectedRoom + '/consult'" method="POST" class="p-6">
                    @csrf
                    <div class="mb-5">
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Message about Room <span x-text="selectedRoomName" class="text-primary-400"></span></label>
                        <textarea name="message" rows="4" required class="w-full rounded-xl border-slate-700 bg-slate-800 text-white placeholder-slate-500 focus:ring-primary-500 focus:border-primary-500 transition-all" placeholder="Ask about room availability, cleaning permission..."></textarea>
                    </div>
                    <div class="flex justify-end gap-3">
                        <button type="button" @click="showConsultModal = false" class="px-5 py-2.5 text-sm font-bold text-slate-400 hover:text-white transition-colors">Cancel</button>
                        <button type="submit" class="bg-primary-600 hover:bg-primary-700 text-white font-bold px-6 py-2.5 rounded-xl shadow-lg shadow-primary-600/20 transition-all">Send Message</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <template x-if="showIssueModal">
        <div class="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <div class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm" @click="showIssueModal = false"></div>
            <div class="bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl w-full max-w-md relative z-10 overflow-hidden">
                <div class="bg-red-600 p-6 text-white flex justify-between items-center border-b border-red-700">
                ...
                <form :action="'{{ url('housekeeping/rooms') }}/' + selectedRoom + '/issue'" method="POST" class="p-6">
                    @csrf
                    <div class="mb-5">
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Issue in Room <span x-text="selectedRoomName" class="text-red-400"></span></label>
                        <textarea name="description" rows="4" required class="w-full rounded-xl border-slate-700 bg-slate-800 text-white placeholder-slate-500 focus:ring-red-500 focus:border-red-500 transition-all" placeholder="Describe the problem (e.g., Broken AC, Leaking tap...)"></textarea>
                    </div>
                    <div class="flex justify-end gap-3">
                        <button type="button" @click="showIssueModal = false" class="px-5 py-2.5 text-sm font-bold text-slate-400 hover:text-white transition-colors">Cancel</button>
                        <button type="submit" class="bg-red-600 hover:bg-red-700 text-white font-bold px-6 py-2.5 rounded-xl shadow-lg shadow-red-600/20 transition-all">Submit Report</button>
                    </div>
                </form>
            </div>
        </div>
                    ...
                    @forelse($orders as $order)
                        <tr class="hover:bg-white/5 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="font-mono text-sm font-bold text-accent-400 bg-accent-500/10 border border-accent-500/20 px-2.5 py-1 rounded-lg">#{{ $order->id }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="font-semibold text-white text-sm">{{ $order->user->name }}</div>
                                <div class="text-xs text-slate-400">Room: {{ $order->room ? $order->room->name : 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap font-bold text-white text-sm">{{ $order->service->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-3 py-1 text-xs font-bold rounded-full
                                    @if($order->status == 'pending') bg-amber-500/10 text-amber-500 border border-amber-500/20
                                    @elseif($order->status == 'confirmed') bg-primary-500/10 text-primary-400 border border-primary-500/20
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <form action="{{ route('housekeeping.orders.update', $order) }}" method="POST" class="inline-block">
                                    @csrf @method('PATCH')
                                    <select name="status" onchange="this.form.submit()" class="rounded-xl border-slate-700 bg-slate-800 text-white text-sm focus:border-accent-500 focus:ring-accent-500 py-1.5 px-2 transition-all">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="w-12 h-12 bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-3 border border-slate-700">
                                        <svg class="w-6 h-6 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                    </div>
                                    <p class="text-sm text-slate-500 italic">No service requests found.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div id="inventory" class="scroll-mt-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <button @click="showAddModal = true" class="bg-primary-600 hover:bg-primary-700 text-white font-bold py-2 px-4 rounded-xl shadow-lg shadow-primary-600/20 transition-all hover:-translate-y-0.5 flex items-center gap-2 text-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @forelse($inventoryItems ?? [] as $item)
                <div class="bg-slate-900/50 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-800 p-6 hover:border-primary-500/30 transition-all duration-300 relative overflow-hidden group">
                    ...
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h3 class="font-bold text-lg text-white group-hover:text-primary-400 transition-colors line-clamp-1" title="{{ $item->name }}">{{ $item->name }}</h3>
                    ...
                    <button @click="selectedItem = '{{ $item->id }}'; selectedItemName = '{{ str_replace('\'', '\\\'', $item->name) }}'; selectedItemMax = {{ $item->quantity }}; showUseModal = true"
                        class="w-full bg-slate-800 hover:bg-primary-500 text-white font-bold py-2.5 rounded-xl text-xs uppercase tracking-widest border border-slate-700 hover:border-primary-400 transition-all flex items-center justify-center gap-2"
                ...
                <form action="{{ route('housekeeping.inventory.store') }}" method="POST" class="p-6">
                    ...
                    <div class="space-y-4 mb-6">
                        <div>
                            ...
                            <input type="text" name="name" required class="w-full rounded-xl border-slate-700 bg-slate-800 text-white placeholder-slate-500 focus:ring-primary-500 focus:border-primary-500 transition-all">
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Initial Qty</label>
                                <input type="number" name="quantity" min="0" required class="w-full rounded-xl border-slate-700 bg-slate-800 text-white focus:ring-primary-500 focus:border-primary-500 transition-all">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Unit (eg. bottles, pcs)</label>
                                <input type="text" name="unit" class="w-full rounded-xl border-slate-700 bg-slate-800 text-white placeholder-slate-600 focus:ring-primary-500 focus:border-primary-500 transition-all">
                            </div>
                        </div>
                    </div>
                    <div class="flex justify-end gap-3">
                        ...
                        <button type="submit" class="bg-primary-600 hover:bg-primary-700 text-white font-bold px-6 py-2.5 rounded-xl shadow-lg shadow-primary-600/20 transition-all">Save Item</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="fixed inset-0 z-[100] flex items-center justify-center px-4">
            ...
            <div class="bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl w-full max-w-md relative z-10 overflow-hidden">
                ...
                <form :action="'{{ url('housekeeping/inventory') }}/' + selectedItem + '/use'" method="POST" class="p-6">
                    @csrf
                    <div class="mb-5 bg-primary-500/10 text-primary-400 p-4 rounded-2xl border border-primary-500/20 flex items-center gap-3">
                    ...
                    <div class="space-y-4 mb-6">
                        <div>
                            ...
                            <input type="number" name="quantity" step="0.01" min="0.01" :max="selectedItemMax" required class="w-full text-2xl font-black py-4 text-center rounded-xl border-slate-700 bg-slate-800 text-white focus:ring-primary-500 focus:border-primary-500 transition-all">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Reason / Description</label>
                            <textarea name="description" rows="2" required class="w-full rounded-xl border-slate-700 bg-slate-800 text-white placeholder-slate-600 focus:ring-primary-500 focus:border-primary-500 transition-all" placeholder="e.g., Replaced towels in Room 101."></textarea>
                        </div>
                    </div>
                    <div class="flex justify-end gap-3">
                        ...
                        <button type="submit" class="bg-primary-600 hover:bg-primary-700 text-white font-bold px-6 py-2.5 rounded-xl shadow-lg shadow-primary-600/20 transition-all">Log Usage</button>
                    </div>
                </form>
            </div>
        </div>
