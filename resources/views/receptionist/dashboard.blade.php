...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <h1 class="text-3xl font-bold text-slate-800 mb-8">Receptionist Dashboard</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="text-sm font-medium text-slate-500">Today's Check-ins</div>
            <div class="text-3xl font-bold text-green-600 mt-1">{{ $stats['today_checkins'] }}</div>
        </div>
        <div class="bg-white rounded-lg shadow p-6">
            <div class="text-sm font-medium text-slate-500">Today's Check-outs</div>
            <div class="text-3xl font-bold text-sky-600 mt-1">{{ $stats['today_checkouts'] }}</div>
        </div>
        <div class="bg-white rounded-lg shadow p-6">
    ...
    <div class="mb-8 flex gap-4">
        <a href="{{ route('receptionist.bookings') }}" class="bg-orange-600 text-white px-6 py-2 rounded-md hover:bg-orange-700 font-medium">
            Manage Bookings
        </a>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-200">
            <h2 class="text-xl font-semibold text-slate-800">Upcoming Bookings</h2>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200">
                ...
                <tbody class="divide-y divide-slate-200">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-slate-500">No upcoming bookings.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="px-6 py-3 bg-slate-50 border-t">
            <a href="{{ route('receptionist.bookings') }}" class="text-orange-600 hover:text-orange-800 text-sm font-medium">View all bookings →</a>
        </div>
    </div>
</div>
@endsection
