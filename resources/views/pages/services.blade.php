            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-16">
                ...
                @foreach($services as $service)
                    <div class="group bg-white rounded-[3rem] shadow-[0_20px_50px_rgba(11,16,33,0.05)] border border-slate-50 p-16 flex flex-col items-center text-center hover:shadow-[0_40px_80px_rgba(11,16,33,0.12)] hover:-translate-y-4 transition-all duration-700 animate-slide-up"
                        ...
                        <h5 class="text-3xl font-bold text-navy mb-6 font-serif group-hover:text-gold transition-colors">{{ $service['title'] }}</h5>
                        <p class="text-slate-500 font-light leading-relaxed mb-10 flex-grow text-sm">{{ $service['desc'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>
