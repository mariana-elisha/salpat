...
<div class="space-y-8">
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <h2 class="text-2xl font-serif font-bold text-white flex items-center gap-4">
                <span class="w-1 h-8 bg-gold rounded-full shadow-[0_0_15px_rgba(212,175,55,0.5)]"></span>
                Room Reservations
            </h2>
            <a href="{{ route('rooms.index') }}" class="text-[10px] font-bold text-gold uppercase tracking-[0.2em] hover:text-white transition-colors">Book Another Room →</a>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            @forelse($bookings as $booking)
                <div class="group bg-slate-900/50 backdrop-blur-xl rounded-[2rem] shadow-2xl border border-white/5 overflow-hidden hover:border-gold/50 transition-all duration-500 relative">
                    <div class="absolute top-0 left-0 w-2 h-full {{ $booking->status == 'confirmed' ? 'bg-gold shadow-[0_0_10px_rgba(212,175,55,0.5)]' : 'bg-slate-500' }}"></div>
                    <div class="p-8 relative z-10">
                        <div class="flex justify-between items-start mb-6">
                            <div>
                                <h3 class="text-xl font-bold text-white group-hover:text-gold transition-colors font-serif">
                                    {{ $booking->room->name }}
                                </h3>
                                <div class="flex items-center gap-3 mt-2">
                                    <span class="text-[10px] font-bold text-gold uppercase tracking-widest capitalize">{{ $booking->room->type }}</span>
                                    <span class="w-1 h-1 rounded-full bg-slate-600"></span>
                                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $booking->check_in->format('M d') }} — {{ $booking->check_out->format('M d, Y') }}</span>
                                </div>
                            </div>
                            <span class="px-3 py-1 text-[8px] font-bold rounded-full uppercase tracking-widest border
                                {{ $booking->status == 'confirmed' ? 'bg-gold/10 text-gold border-gold/30' : 'bg-slate-800 text-slate-400 border-slate-700' }}">
                                {{ $booking->status }}
                            </span>
                        </div>
                        
                        <div class="bg-slate-800/50 rounded-2xl p-6 border border-slate-700 mb-6 flex items-center justify-between">
                            <div>
                                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Booking Total</p>
                                <div class="flex items-baseline gap-2">
                                    <p class="text-3xl font-serif font-bold text-white">${{ number_format($booking->total_price, 2) }}</p>
                                    <p class="text-[10px] text-slate-500 font-bold uppercase">{{ number_format($booking->total_price * \App\Models\Room::USD_TO_TZS, 0) }} TZS</p>
                                </div>
                            </div>
                            <div class="w-12 h-12 bg-slate-800 rounded-xl shadow-sm flex items-center justify-center text-gold border border-slate-700">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            </div>
                        </div>

                        <div class="flex items-center justify-between pt-2">
                            <span class="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">ID: {{ $booking->booking_reference }}</span>
                            <a href="{{ route('bookings.show', $booking) }}" class="text-[10px] font-bold text-white hover:text-gold transition-colors flex items-center gap-2 group/btn uppercase tracking-widest">
                                Manage Stay
            ...
            @empty
                <div class="col-span-full bg-slate-900/30 rounded-[2rem] border-2 border-dashed border-slate-700 p-16 text-center">
                    <div class="w-16 h-16 bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-700">
                        <svg class="w-8 h-8 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
                    </div>
                    <p class="font-semibold text-slate-300 mb-2">No active bookings</p>
                    <a href="{{ route('rooms.index') }}" class="text-gold font-bold hover:text-white transition-colors text-sm">Browse available rooms →</a>
                </div>
            @endforelse
        </div>
    </div>
                ...
                <tbody class="divide-y divide-slate-800">
                    @forelse($orders as $order)
                        <tr class="hover:bg-white/5 transition-colors">
                            <td class="px-6 py-4">
                                <span class="font-semibold text-white">{{ $order->service->name }}</span>
                                <span class="text-xs text-slate-400 block lowercase">{{ $order->service->type }}</span>
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-300 font-bold">× {{ $order->quantity }}</td>
                            ...
                            <td class="px-6 py-4">
                                <span class="text-xs font-bold px-3 py-1 rounded-full
                                    ...
                                    @else bg-slate-800 text-slate-400 border border-slate-700 @endif uppercase">
                                    {{ $order->status }}
                                </span>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-slate-500 text-sm italic">You haven't ordered any services yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
