...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-serif font-bold text-slate-900">Food Orders</h1>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200">
                    ...
                    <tbody class="bg-white divide-y divide-slate-200">
                        @forelse($orders as $order)
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                                    ...
                                    <div class="text-xs text-slate-400">Room: {{ $order->room ? $order->room->name : 'N/A' }}
                                    </div>
                                </td>
                                    ...
                                    <form action="{{ route('chef.orders.update', $order) }}" method="POST" class="inline-block">
                                        ...
                                        <select name="status" onchange="this.form.submit()"
                                            ...
                                            <option value="confirmed" {{ $order->status == 'confirmed' ? 'selected' : '' }}>
                                                Confirm</option>
                                            <option value="completed" {{ $order->status == 'completed' ? 'selected' : '' }}>Done
                                            </option>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-slate-400 italic">No food orders found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="mt-10 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="divide-y divide-slate-100">
                ...
                @empty
                    <div class="px-6 py-10 text-center text-slate-400 text-sm italic">You haven't submitted any reports yet.
                    </div>
                @endforelse
            </div>
        </div>
