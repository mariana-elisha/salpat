...
<?php $__env->startSection('content'); ?>
    ...
    <div class="relative h-[65vh] min-h-[500px] w-full overflow-hidden bg-slate-900 group" 
         x-data="{ activeImage: 0, images: <?php echo e(json_encode($room->images ? array_merge([$room->image], $room->images) : [$room->image])); ?> }">
        ...
        <template x-for="(img, index) in images" :key="index">
            <div class="absolute inset-0 w-full h-full" x-show="activeImage === index" 
                ...
                <img :src="'/storage/' + img" :alt="'<?php echo e($room->name); ?> image ' + (index + 1)"
                    class="h-full w-full object-cover object-center opacity-70">
            </div>
        </template>
        
        <?php if(!$room->image && (!$room->images || count($room->images) == 0)): ?>
            <img src="https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80"
                alt="<?php echo e($room->name); ?>" class="h-full w-full object-cover object-center opacity-70">
        <?php endif; ?>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-30 pb-24">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div class="lg:col-span-2 space-y-12">
                ...
                <div class="bg-white rounded-[2rem] p-8 md:p-10 shadow-xl border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                        <nav class="flex mb-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 gap-2">
                            <a href="<?php echo e(route('home')); ?>" class="hover:text-accent-500 transition-colors">Home</a>
                            <span>/</span>
                            <a href="<?php echo e(route('rooms.index')); ?>" class="hover:text-accent-500 transition-colors">Rooms</a>
                        </nav>
                        <h2 class="text-3xl font-serif font-bold text-slate-900"><?php echo e($room->name); ?></h2>
                    </div>
                    <div class="text-right">
                        <div class="flex items-baseline gap-2">
                            <span class="text-4xl font-bold text-accent-500">$<?php echo e(number_format($room->price_per_night, 0)); ?></span>
                            <span class="text-slate-400 text-sm font-medium">/ night</span>
                        </div>
                        <?php if($room->resident_price_per_night): ?>
                        <div class="mt-1 flex items-center justify-end gap-2 text-xs font-bold text-emerald-600">
                            TZS <?php echo e(number_format($room->resident_price_per_night, 0)); ?> (Resident)
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-[2rem] p-8 md:p-12 shadow-xl border border-slate-100">
                    ...
                    <div class="prose prose-slate max-w-none text-slate-600 leading-relaxed text-lg font-light mb-12">
                        <?php echo e($room->description ?? 'Experience the perfect blend of comfort and serenity. This room is designed to provide a peaceful retreat, featuring premium amenities and stunning views.'); ?>
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-accent-200 transition-colors group">
                            ...
                            <span class="text-sm font-medium text-slate-700"><?php echo e($amenity); ?></span>
                        </div>
            ...
            <div class="lg:col-span-1" id="whatsapp-sidebar"
                 x-data="{
                    checkIn: '<?php echo e(date('Y-m-d')); ?>',
                    checkOut: '<?php echo e(date('Y-m-d', strtotime('+1 day'))); ?>',
                    guests: 1,
                     roomName: '<?php echo e(addslashes($room->name)); ?>',
                     roomNumber: '<?php echo e($room->room_number ?? ''); ?>',
                     roomType: '<?php echo e(ucfirst($room->type)); ?>',
                     priceUsd: '<?php echo e(number_format($room->price_per_night, 0)); ?>',
                     priceTzs: '<?php echo e(number_format($room->tzs_price, 0)); ?>',
                     residentPrice: '<?php echo e($room->resident_price_per_night ? number_format($room->resident_price_per_night, 0) : ''); ?>',
                     amenities: <?php echo e(json_encode($room->amenities ?? [])); ?>,
                     get whatsappUrl() {
                        ...
                        <div class="p-8 text-white relative">
                            ...
                            <div class="space-y-4">
                                <div class="flex items-baseline justify-between border-b border-slate-800 pb-3">
                                    ...
                                    <div class="text-right">
                                        <span class="text-2xl font-serif font-bold text-white">$<?php echo e(number_format($room->price_per_night, 0)); ?></span>
                                ...
                                <div class="flex items-baseline justify-between">
                                    ...
                                    <div class="text-right">
                                        <span class="text-xl font-serif font-bold text-accent-400"><?php echo e(number_format($room->resident_price_per_night, 0)); ?></span>
                        ...
                        <div class="p-8 bg-white space-y-5">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="date" x-model="checkIn" min="<?php echo e(date('Y-m-d')); ?>"
                                ...
                                <div>
                                    ...
                                    <input type="date" x-model="checkOut" min="<?php echo e(date('Y-m-d', strtotime('+1 day'))); ?>"
                                        class="w-full px-3 py-3 border border-slate-200 rounded-xl focus:ring-accent-500 focus:border-accent-500 text-sm font-medium bg-slate-50/50">
                                </div>
                            </div>
                                ...
                                <div class="relative">
                                    <select x-model="guests" class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-accent-500 focus:border-accent-500 bg-slate-50/50 appearance-none font-medium pr-10">
                                        <?php for($i = 1; $i <= $room->capacity; $i++): ?>
                                            <option value="<?php echo e($i); ?>"><?php echo e($i); ?> <?php echo e($i > 1 ? 'Guests' : 'Guest'); ?></option>
                                        <?php endfor; ?>
                                    </select>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\salpat\resources\views/rooms/show.blade.php ENDPATH**/ ?>